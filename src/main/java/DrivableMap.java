...
class DrivableMap {
    ...
    public boolean hasFasterThan(int speed) {
        List<Drivable> values = new ArrayList<>(drivable_map.values());
        for (int i = 0; i < values.size(); i++){
            if (values.get(i).getMaxSpeed() >= speed){
                return true;
            }
        }
    return false;
    ...
    public List<Tradable> getTradable(){
        ...
        for (int i = 0; i < values.size(); i++){
            if (values.get(i) instanceof Tradable){
                tradables.add((Tradable) values.get(i));
            }
        }
        return tradables;
    }
